...
<head>
    ...
    <title>Revenue Records Mapping - {{ $title ?? '' }}</title>
                    ...
                    @foreach ($menu_r as $menu)
                        @if ($menu->is_collapsible == 'no')
                            <li class="menu-item">
                                ...
                                @endphp
                                @if ($firstPermission)
                                    <a href="{{ route($firstPermission->routes[0]->route) }}"
                                        class="menu-link @if (\Illuminate\Support\Facades\Route::currentRouteName() == $firstPermission->routes[0]->route) active @endif">
                                        <i class="menu-icon tf-icons {{ $menu->icon }}"></i>
                                        <span
                                            data-i18n="{{ $firstPermission->routes[0]->title }}">{{ $firstPermission->routes[0]->title }}</span>
                                    </a>
                                @endif
                            </li>
                        @else
