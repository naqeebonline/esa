    ...
    <div class="layout-wrapper layout-content-navbar">
        ...
        <li class="nav-item dropdown-notifications navbar-dropdown dropdown me-3 me-xl-2">
            <a class="nav-link dropdown-toggle hide-arrow" href="javascript:void(0);" data-bs-toggle="dropdown" data-bs-auto-close="outside" aria-expanded="false">
                ...
                <span class="badge bg-danger rounded-pill badge-notifications" id="user_notification_count">{{ getFMCUserNotifictions()['count']  ?? 0}}</span>
            </a>
            <ul class="dropdown-menu dropdown-menu-end py-0">
                ...
                <li class="dropdown-notifications-list scrollable-container">
                    <ul class="list-group list-group-flush" id="notifications-container">
                        @foreach(getFMCUserNotifictions()['notifications'] ?? [] as $notification)
                        <li class="list-group-item list-group-item-action dropdown-notifications-item">

                            <div class="d-flex">
                                <div class="flex-shrink-0 me-3">
                                    <div class="avatar">
                                        <img src="{{$notification->seller?->getFirstMediaUrl('seller_image') ?? ''}}" alt class="w-px-40 h-auto rounded-circle" />
                                    </div>
                                </div>
                                <div class="flex-grow-1">
                                    <a href="{{route('fmc.complaints.show', ['id' =>   \Illuminate\Support\Facades\Crypt::encrypt($notification->complaint_id)])}}" class="text-body">
                                        <h6 class="mb-1">{{$notification->title ?? ''}}</h6>
                                        <p class="mb-0">{{$notification?->complaint?->user?->company?->title ?? ''}}</p>
                ...
                <li class="dropdown-menu-footer border-top">
                    <a href="{{route('fmc.complaints.match')}}" class="dropdown-item d-flex justify-content-center p-3">
        ...
        <li class="nav-item navbar-dropdown dropdown-user dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end">
                ...
                <li>
                    <a class="dropdown-item" href="{{route('fmc.users-mgt.my-profile')}}">
                        <i class="bx bx-user me-2"></i>
                        <span class="align-middle">My Profile</span>
                    </a>
                </li>
                <li>
                    <a class="dropdown-item" href="{{route('fmc.users-mgt.change-password')}}">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            function fetchNotifications() {
                $.ajax({
                    url: "{{route('fmc.notifications.unread')}}",
                    method: 'GET',
                    success: function(response) {

                        $('#user_notification_count').text(response?.data?.total);
                        // Clear all content inside the #notifications-container
                        $('#notifications-container').empty();

                        response?.data?.data.forEach(function(notification) {
                            var timestamp = moment(notification.created_at);
                            ...
                            <li class="list-group-item list-group-item-action dropdown-notifications-item">
                                <div class="d-flex">
                                    <div class="flex-shrink-0 me-3">
                                        <div class="avatar">
                                            <img src="${notification?.seller?.image_url || ''}" alt class="w-px-40 h-auto rounded-circle" />
                                        </div>
                                    </div>
                                    <div class="flex-grow-1">
                                    <a href="` + notification?.show_url + `" class="text-body">
                                        <h6 class="mb-1">${notification?.title}</h6>
                        ...
                        `);
                            ...
                            // Append the generated liElement to the container
                            $('#notifications-container').append(liElement);
                        });
                    },
            ...
            // Fetch notifications initially on page load
            // fetchNotifications();

            // Optionally, you can set up a periodic refresh using setInterval
            setInterval(fetchNotifications, 60000); // Refresh every 1 minute
        });
    </script>
